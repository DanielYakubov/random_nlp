...
def show_bar_graph(data: dict) -> None:
    """displays values for a dictionary containing
    k:v pairs where v is a list containing 2 items"""
    keys, values_list = list(data.items())
...
if __name__ == '__main__':
    wrds = words.words()
    brwn = brown.words() # don't want to remove stop words as we are looking a letter frequencies.
    ...
    plt.bar(bar1, wrds_values, color='g', width=width,
            edgecolor='black', label='Words Corpus')
    plt.bar(bar2, brwn_values, color='b', width=width,
            edgecolor='black', label='Brown Corpus')
    plt.xlabel('Letter', fontweight='bold', fontsize=12)
    plt.ylabel('Frequency as a %', fontweight='bold', fontsize=12)
    plt.xticks([r + width for r in range(len(wrds_values))],
               letters)
    plt.legend()
    plt.show()
